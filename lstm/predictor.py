...
class Predictor:
    ...
    def score(self, context: torch.Tensor, candidate_keys: list[int]) -> dict[int, float]:
        ...
        candidate_features = [
            self.candidate_builder.build(key) for key in candidate_keys
        ]
        candidate_tensor = torch.stack([
            torch.as_tensor(candidate_to_array(cf), dtype=torch.float32)
        ...
        predictions = predictions.expm1()  # invert log1p — model was trained on log1p(label)
